# Логистическая регрессия с "мешком слов" по героям
...
import re
players = [name for name in data.columns if re.search('r._hero', name)] + \
[name for name in data.columns if re.search('d._hero', name)]
heroes = data[players].stack().value_counts()
heroes_num = max(heroes.index)
used_heroes_num = heroes.shape[0]
print('Number of unique heroes is', heroes_num, ',', used_heroes_num, 'among them were used')
# Создание "мешка слов"
